...
export default function reducer(state = [], action = {}) {
  switch (action.type) {
    ...
    case RESERVE_MOTOCYCLE:
      return state.map(
        (motocycle) => (motocycle.id !== action.motocycle.id ? motocycle : { ...motocycle, reserved: true }),
      );
    case CANCEL_RESERVATION:
      return state.map(
        (motocycle) => (motocycle.id !== action.motocycle.id ? motocycle : { ...motocycle, reserved: false }),
      );
    default:
      return state;
  }
